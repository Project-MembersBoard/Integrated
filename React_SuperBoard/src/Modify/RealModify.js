...
export default function ModifyUserUI() {
    ...
    // 새 이메일 용도
    const [newEmail, setNewEmail] = useState({nemail:"", authcode:""});
 
    // 비밀번호 변경 창 띄우는 용도의 state
    const [showNewPasswordRow, setShowNewPasswordRow] = useState(false);

    // 이메일 변경 창 띄우는 용도의 state
    const [showNewemailRow, setShowNewEmailRow] = useState(false);
    ...
    const printModifyEmail = (e) => {
        e.preventDefault();
        setShowNewEmailRow(true);
    };
    ...
    const getAuthcode = async (e) => {
        ...
        try {
            ...
            body: JSON.stringify({ email: newEmail.nemail }),
            credentials: "include"
        });
        ...
        if (data.available) {
            alert('이메일: 사용 가능한 이메일입니다.');
            alert("메일이 전송되었습니다");
        } else {
            alert('이메일: 이미 사용중인 이메일입니다.');
            return;
        }
        } catch (error) {
            alert(error.message);
    ...
    const AuthenticationCode = async (e) => {
        ...
        try{
            ...
            const response = await fetch(`${URL}/email/verify`, {
                ...
                body: JSON.stringify({
                    email: newEmail.nemail,
                    code: newEmail.authcode
                }),
                credentials: "include"
            });
    ...
    const getUserData = async () => {
        ...
        try {
            const response = await fetch(`${URL}/modify/getUserData`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const goUpdate = async(e) => {
        ...
        if(!availableid || !availablepw || !availablemobile || !availableemail) {
            alert("인증을 다 받으십시오");
            return;
        }
        ...
        try{
            ...
            const response = await fetch(`${URL}/modify/updateInfo`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleDelete = async (e) => {
        ...
        try{
            ...
            const response = await fetch(`${URL}/modify/deleteUser`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                ...
                <table className="ModifyUserUITable">
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <input type="text" className="ModifyUserUITableText" onChange={setValueId} value={modifyvalue.userid} name="userid" />
                            </td>
                            <td>
                                <input type="button" className="ModifyUserUITalbeBtn" onClick={checkNewId} value="변경" />
                            </td>
                        </tr>
                        <tr>
                            <td>
                        ...
                        {showNewPasswordRow && (
                            <>
                                <tr>
                                    ...
                                    <td colSpan="2">
                                        <input type="password" className="ModifyUserUITableText2" value={newpw.cpw} onChange={currentPassword} name="cpw" />
                                    </td>
                                    <td></td>
                                </tr>
                                <tr>
                                    ...
                                    <td colSpan="2">
                                        <input type="password" className="ModifyUserUITableText2" value={newpw.npw} onChange={newPassword} name="npw" />
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    ...
                                    <td colSpan="2">
                                        <input type="password" className="ModifyUserUITableText2" value={newpw.chpw} onChange={newPassword} name="chpw" />
                                    </td>
                                </tr>
                                <tr>
                                    <td colSpan="3">
                        ...
                        <tr>
                            <td>
                                <label className="ModifyUserUITableLabel">이메일</label>
                            </td>
                            <td>
                                <input type="text" className="ModifyUserUITableText" value={first.email} name="email" readOnly />
                            </td>
                            <td>
                                <input type="button" className="ModifyUserUITalbeBtn" onClick={printModifyEmail} value="변경" />
                            </td>
                        </tr>
                        {/* 버튼 클릭 시 새로 추가될 tr */}
                        {showNewemailRow && (
                            <>
                                <tr>
                                    <td>
                                        <label className="ModifyUserUITableLabel">새 이메일</label>
                                    </td>
                                    <td>
                                        <input type="text" className="ModifyUserUITableText" onChange={inputNewEmail} name="nemail" />
                                    </td>
                                    <td>
                                        <input type="button" className="ModifyUserUITableRowBtn" onClick={getAuthcode} value="인증번호받기" />
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <label className="ModifyUserUITableLabel">인증번호</label>
                                    </td>
                                    <td>
                                        <input type="text" className="ModifyUserUITableText" onChange={inputNewEmail} name="authcode" />
                                    </td>
                                    <td>
                                        <input type="button" className="ModifyUserUITableRowBtn" onClick={AuthenticationCode} value="인증확인" />
                                    </td>
                                </tr>
                                <tr>
                                    <td></td>
                                    ...
                                    <td>
                                        <input type="button" className="ShowNewPasswordBtn2" value="초기화" />
                                    </td>
                                </tr>
                            </>
                        )}
